...
/*   Updated: 2023/03/13 19:51:15 by hyeonsul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (n[0]->num > n[0]->next->num)
		return (n[0]->next->num > n[1]->num || n[0]->num < n[1]->num);
	else
		return (n[0]->num < n[1]->num && n[0]->next->num > n[1]->num);
}

void	loop_a(t_deque *dq, t_node **n, t_cmd_info *cmd_info, size_t b_i)
{
...
void	set_cmd(t_deque *dq, t_cmd_info *cmd_info, size_t b_i)
{
